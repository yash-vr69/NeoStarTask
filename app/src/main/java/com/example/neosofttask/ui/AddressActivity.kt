    ...
    private fun setObservers() = lifecycleScope.launch{
        ...
        viewModel.apply {
            launch {
                ...
                errorMsg.collectLatest{
                    displayErrorToast(it.first)
                    when(it.second){
                       ErrorTypes.ADDRESS -> {
                           binding.addressEdtTxt.error = it.first
                       }
                        ErrorTypes.LANDMARK -> {
                           binding.landmarkEdtTxt.error = it.first
                       }
                        ErrorTypes.CITY -> {
                           binding.cityEdtTxt.error = it.first
                       }
                        ErrorTypes.STATE -> {

                       }
                        ErrorTypes.PINCODE -> {
                           binding.pinCodeEdtTxt.error = it.first
                       }
                    }
                }
            }
